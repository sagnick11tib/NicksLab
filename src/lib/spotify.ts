...
const basic = Buffer.from(`${client_id}:${client_secret}`).toString("base64"); //it is for basic auth
...
export const getNowPlaying = async (): Promise<NowPlayingData | string | null> => {
  try {
    const { access_token } = await getAccessToken();

    if (!access_token) throw new Error("Unable to get access token");
